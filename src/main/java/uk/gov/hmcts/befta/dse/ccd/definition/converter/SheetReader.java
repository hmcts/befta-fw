...
public class SheetReader {
     ...
     * @return ArrayNode object containing array of json objects, each item in array
     *         is a row from the definition sheet
     */
    ...
    private boolean rowEmpty(ObjectNode objectNodeForRow) {
        ...
        Iterator<String> it = objectNodeForRow.fieldNames();
        boolean empty = true;
        while (it.hasNext()) {
            String key = it.next();
    ...
    /**
     * If a sheet has no valid rows we insert a single empty row equivalent in the
     * json structure so we can keep a record of the headers for purposes of a json
     * -> xlsx transformation
     * 
     * @return
     */
     ...
     */
    private ArrayList<String> getHeaders(Sheet sheet) {
        ArrayList<String> header = new ArrayList<String>();
        Iterator<Cell> cells = sheet.getRow(HEADER_ROW).cellIterator();
        while (cells.hasNext()) {
     ...
     * @return ObjectNode representation for single excel row in definition file, if
     *         empty row is detected as all cells are empty, return null instead
     */
    ...
    /**
     * Get celltype, ccd Custom date field throws a null pointer exception as poi
     * library cannot detect 'Custom' cell type so we catch and model as blank and
     * handle later by column/header name
     * 
     * @param cell
     * @return CellType
     */
