...
public interface BackEndFunctionalTestAutomationDSL {
    ...
    // DSL Element:
    // "It is submitted to call the [<some operation name>] operation of [<some application / api product name>]
    // [with a delay of <delay duration> seconds <before/after> the request]."
    void submitTheRequestToCallAnOperationOfAProductWithDelay(String operation, String productName, int delay,
    ...
    // And a successful call [<some specification about status>] until the expected response is received [<some operation name>] within a timeout of [<source>]
    void performAndVerifyTheExpectedResponseForAnApiCallWithHmiWait(String testDataSpec, String testDataId, String source) throws IOException;
